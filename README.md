...
|---|---|---|
| 影视飓风 — a *sponsored* video | 9.8M (a hit) | The ad was produced to the same signature standard as their normal work. |
| 妈咪说 — a livestream re-upload | 17.6K (a flop) | Below-signature *production*, not below-signature *topic*. |
...
*lower* plays (极客湾 0.30×, 王刚 0.39×, 老番茄 0.54×). `「你」` is a *knowledge-explainer* hook
...
points *opposite* ways for two creators (钟文泽 −0.55 vs 盗月社 +0.52 — same ruler, reversed sign).
...
We re-ran the *identical* form test on a new sample — **MrBeast, Dude Perfect, Airrack, Ryan Trahan**
(4 creators · `Entertainment-YT` zone · 160 long-form videos, Shorts excluded). The same top5/bot5
gap held — without a single counter-case:

| Creator | top5/bot5 | Signature form (top5 common) | Off-format (bot5 common) |
|---|---|---|---|
| MrBeast | **3.5×** (tightest) | `Win + $X` + extreme-constraint challenges | charity (`Feed 1 Person` / `10 Schools`) + introspection (`Hi Me In 10 Years`) |
| Dude Perfect | 6.3× | celebrity-vs-Dude duels + team set-pieces | `OVERTIME 50` series fatigue + solo NBA shoot |
| Airrack | 6.0× | `I Secretly Lived In X` template (MrBeast's house, Kai Cenat's house, grocery) | non-Secretly bouts (boxing, "100 Worst Deaths" experiment) |
| Ryan Trahan | **8.8×** | single-shot extreme-location pieces (longest bus, abandoned mall, $X cruise) | `Top 10 Places in America` Day 3 → Day 6 — **4 consecutive flops** |
...
The cleanest single case is **Ryan Trahan**. His signature single-shot extreme-location videos
average ~17M plays. The moment he switches to a *daily diary* format, `Top 10 Places in America
Day 3 → Day 6` land **2.5M / 2.0M / 1.7M / 1.1M** — 4 videos in a row at ~10% of his ceiling,
...
MrBeast and peers stuff brand mentions like `podcast` / `interview` (a false positive on virtually
every English video). Fix: skip `description` on YouTube; drop the over-broad `livestream` /
`live stream` tokens that mis-tagged Airrack's flagship `I Secretly Hid in Celebrity Livestreams`
series.
...
because `play_per_day` inflates new uploads) caught two real declines: **itsRae** (recent/early =
0.07×, ρ=−0.68) and **王刚** (0.66×, ρ=−0.30 *while posting more often* — the classic
...
- **"Signature form" must be defined *per creator*.** Off-format tagging uses generic keyword markers
  (`商单 / vlog / 访谈 / 直播 …`). That mis-tags someone like **itsRae, whose signature literally *is*
...
**跨语种扩展(2026-05-29):** 同一个 form 测试搬到英文 YouTube — **MrBeast / Dude Perfect /
Airrack / Ryan Trahan** 4 人 160 条长视频(Shorts 已排除)。spread **3.5–8.8×**,全部落在原
3.7–109× 区间内,**无一例外**。最干净的反面教材是 Ryan Trahan:招牌单条极限地点视频均播放
约 1700 万,但他切到"日记连载"形式后,`Top 10 Places in America Day 3 → Day 6` 连续 4 集
播放 **250 万 / 200 万 / 170 万 / 110 万** — 同一人、同一观众、同一月,只是 form 变了。

附带还能做:**分区基准**(把你放进同区"典型创作者"里定位)和**疲态检测**(只用满 30 天的成熟
视频总播放判断你在涨还是在跌,已抓到 itsRae、王刚两例真实下滑)。
